...
class KITTI(Dataset):
    ...
    def get_corners(self, bbox: List[float]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Args:
            bbox: [8] list of the bounding box parameters
        ...
        w, h, l, y, z, x, yaw = bbox[8:15]
        y = -y #NOTE: av2 already has y in the right-hand system so no need to negate
    ...
    def update_label_map(self, map: np.ndarray, bev_corners: np.ndarray, reg_target: np.ndarray):
        ...
        Returns:
            None
        """
        # NOTE: bev_corners and reg_target are in metric space of the bird's eye view
        label_corners = (bev_corners / 4 ) / 0.1
        label_corners[:, 1] += self.geometry['label_shape'][0] / 2 # 200/2 = 100
    ...
    # TODO: change this to get the label of av2
    def get_label(self, index: int) -> Tuple[np.ndarray, List[np.ndarray]]:
        '''
...
def find_reg_target_var_and_mean():
    ...
    print("Stds", stds)
    return means, stds
# TODO: Remove this function as not necessary
