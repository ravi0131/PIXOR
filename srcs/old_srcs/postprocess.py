...
def convert_format(boxes_array):
    ...
    for box in boxes_array:
        # Ensure the box forms a closed loop
        if not np.array_equal(box[0], box[-1]):
            box = np.vstack([box, box[0]])
        polygons.append(Polygon([(box[i, 0], box[i, 1]) for i in range(len(box))]))
    return polygons
...
#         i = ixs[0]
#         pick.append(i)
#         # Get remaining polygons using list comprehension
#         remaining_polys = [polygons[ix] for ix in ixs[1:]]
#         # Compute IoU
#         iou = compute_iou(polygons[i], remaining_polys)
...
#         pick.append(i)
#         # Compute IoU of the picked box with the rest
#         iou = compute_iou(polygons[i], [polygons[j] for j in ixs[1:]])
#         # Identify boxes with IoU over the threshold. This
#         # returns indices into ixs[1:], so add 1 to get
#         # indices into ixs.
#         remove_ixs = np.where(iou > threshold)[0] + 1
