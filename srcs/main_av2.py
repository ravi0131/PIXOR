...
from datagen_av2 import get_data_loader
...
def eval_batch(config, net, loss_fn, loader, device):
    net.eval()
    if config['mGPUs']:
    ...
    all_scores = []
    all_matches = []
    log_images = [] # A list of images of point cloud in bev perspective with the predicted bounding boxes
    gts = 0         # number of ground truth boxes
    preds = 0       # number of predicted boxes
    t_fwd = 0
    t_nms = 0

    # log_img_list = random.sample(range(len(loader.dataset)), 10)
    log_img_list = random.sample(range(len(loader.dataset)), 1) #TODO: Remove after overfitting on a single frame
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            predictions = list(torch.split(predictions.cpu(), 1, dim=0)) # predictions is a list containing only a single tensor
            batch_size = len(predictions) # batch size is 1
            ...
            for j in range(batch_size):
                # j = 0
                # batch_size = 1
                # preds_filtered is a list 
                # len(pred_filtered) = 1
                # preds_filterd[0] => ([], [])
                
                # print(f"METHOD: eval_batch =>  preds_filtered[j]: {preds_filtered[j]}") 
                _, label_list = loader.dataset.get_label(image_id[j].item())
                corners, scores = preds_filtered[j] # NOTE: scores are cls logits that are above a certain threshold
                # NOTE: corners is a numpy array of predicted bboxes with shape [N, 4, 2]
                # NOTE: scores is a numpy array of cls logits corresponding to each bbox in `corners` i.e., cls logits for bbox at corners[0] is at scores[0]
                gts += len(label_list)
                preds += len(scores)
                all_scores.extend(list(scores))
                if image_id[j] in log_img_list:
                    input_np = input[j].cpu().permute(1, 2, 0).numpy()
                    pred_image = get_bev(input_np, corners) # pred image is a bev image that can be viewed
            ...
            for j in range(batch_size):
                all_matches.extend(list(matches[j][1])) # why [1]? Because we want the pred_match from compute_matches
            
            #print(time.time() -tic)
    all_scores = np.array(all_scores)
    all_matches = np.array(all_matches)
    sort_ids = np.argsort(all_scores) # all_scores is a 2d numpy array, where each row (logits) are sorted in ascending order
    all_matches = all_matches[sort_ids[::-1]] # what is this doing?
    print(f"METHOD: eval_batch =>  all_matches type: {type(all_matches)}")
    print(f"METHOD: eval_batch =>  all_matches shape: {all_matches.shape}")
    # print(f"METHOD: eval_batch =>  all_matches[0] type: {type(all_matches[0])}")
    print(f"METHOD: eval_batch =>  all_matches: {all_matches}")
    # print(f"METHOD: eval_batch =>  gts type: {type(gts)}")
    # print(f"METHOD: eval_batch =>  preds type: {type(preds)}")
    metrics = {}
...
#NOTE: Main function for model training
def train(exp_name: str, device: torch.device):
    # Load Hyperparameters
    config, learning_rate, batch_size, max_epochs = load_config(exp_name)

    # Dataset and DataLoader
    train_data_loader, test_data_loader = get_data_loader(batch_size,geometry=config['geometry'])
    ...
    for epoch in range(st_epoch, max_epochs):
        ...
        print(f"Epoch {epoch + 1}|Time {time.time() - start_time:.3f}|Training Loss: {train_loss:.5f}")

        # Run Validation
        if (epoch +1) % 10 == 0:
            tic = time.time()
            val_metrics, _, _, log_images = eval_batch(config, net, loss_fn, test_data_loader, device) # Test data loader
            ...
            print(f"Epoch {epoch + 1}|Time {time.time() - tic:.3f}|Validation Loss: {val_metrics['loss']:.5f}")
            print(f"Epoch {epoch + 1}| metrics: {val_metrics}")
        # Save Checkpoint
...
def experiment(exp_name, device, eval_range='all', plot=True):
    ...
    train_loader, val_loader = get_data_loader(config['batch_size'], geometry=config['geometry'])
...
def test(exp_name, device, image_id):
    ...
    net.set_decode(True)
    train_loader, val_loader = get_data_loader(1, geometry=config['geometry'])
    net.eval()
...
def set_seed(seed: int = None):
    """
    Set the random seed for reproducibility. If no seed is provided, a random seed is generated.

    Args:
        seed (int): The seed value to set. If None, a random seed is generated.
    """
    global current_seed
    ...
    current_seed = seed  # Store the seed globally for later use
...
if __name__ == "__main__":
    ...
    set_seed(19036204) #4091515564 lucky number
